...
export default function Profile() {
  ...
  useEffect(() => {
    if (biddedCarsData?.items) {
      setAllBiddedCars((prev) => {
        const newItems = biddedCarsData.items.filter(
          (newCar) => !prev.some((existingCar) => existingCar.carId === newCar.carId)
        );
  ...
  useEffect(() => {
    if (auctionCommentsData?.items) {
      setAllAuctionComments((prev) => {
        const newItems = auctionCommentsData.items.filter(
          (newComment) => !prev.some((existingComment) => existingComment.id === newComment.id)
        );
        return [...prev, ...newItems];
      });
    }
          ...
          <div className="flex-1 max-w-[1072px]">
              ...
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-[22px] font-bold text-black dark:text-white">
                  ...
                  <span className="ml-[5px] text-gray-400 font-normal text-[15px] inline-block whitespace-normal">
                    (Bids of {bidsAndWins?.totalBids || 0} lot, {bidsAndWins?.totalWins || 0} wins)
                  </span>
                </h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                {isBiddedCarsLoading && allBiddedCars.length === 0 ? (
                  <p className="col-span-full text-center text-gray-400">Loading bidded cars...</p>
                ) : biddedCarsError ? (
                  <p className="col-span-full text-center text-red-500">Error loading bidded cars.</p>
                ) : allBiddedCars.length > 0 ? (
                  allBiddedCars.map((car) => (
                    <div
                      key={car.carId}
                    ...
                    >
                      <div className="relative">
                        <img
                          src={car.mainImage || "https://via.placeholder.com/300x200?text=No+Image"}
                          alt={car.carName}
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-gray-500 text-xs mt-3">
                          {new Date(car.bidTime).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                            year: "numeric",
                          })}
                  ...
                  <p className="col-span-full text-center text-gray-400">No bidded cars found.</p>
                )}
              </div>
              {biddedCarsData &&
                biddedCarsData.pageNumber < Math.ceil(biddedCarsData.totalCount / biddedCarsData.pageSize) && (
                  <div className="text-center mt-8">
                    <button
                      onClick={() => setBiddedCarsPage((prev) => prev + 1)}
                      className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors"
                      disabled={isBiddedCarsLoading}
                    >
                      Show more
                    </button>
                  </div>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 min-[518px]:grid-cols-2 gap-6">
                {isCommentsLoading && allAuctionComments.length === 0 ? (
                  <p className="col-span-full text-center text-gray-400">Loading comments...</p>
                ) : commentsError ? (
                  <p className="col-span-full text-center text-red-500">Error loading comments.</p>
                ) : allAuctionComments.length > 0 ? (
                  allAuctionComments.map((comment) => (
                    <div
                      key={comment.id}
                    ...
                    >
                      <div className="w-full h-48 lg:w-[239px] flex-shrink-0 relative">
                        <img
                          src={comment.mainImage || "https://via.placeholder.com/300x200?text=No+Image"}
                          alt={`Auction ${comment.auctionId}`}
                          className="w-full h-full object-cover"
                        />
                      ...
                      <div className="flex-1 flex flex-col justify-between mt-3 p-4 lg:mt-0">
                        <h3 className="text-black dark:text-white text-lg font-bold leading-tight">
                          {comment.year} {comment.make} {comment.model}
                        </h3>
                        <p className="text-gray-400 text-sm mb-auto mt-2">
                  ...
                  <p className="col-span-full text-center text-gray-400">No auction comments found.</p>
                )}
              </div>
              {auctionCommentsData &&
                auctionCommentsData.pageNumber < Math.ceil(auctionCommentsData.totalCount / auctionCommentsData.pageSize) && (
                  <div className="text-center mt-8">
                    <button
                      onClick={() => setAuctionCommentsPage((prev) => prev + 1)}
                      className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors"
