...
import { useChangePasswordMutation } from "@/features/api/endpoints/Profile";
import type { ChangePasswordDto } from "@/features/types/Profile";


interface ChangePasswordModalProps {
  isOpen: boolean;
  onClose: () => void;
  switchTo?: (step: string) => void;
}
...
export function ChangePasswordModal({
  ...
  const handleSubmit = async (values: ChangePasswordDto, { setSubmitting }: any) => {
    try {
          ...
          {({ values, errors, touched, handleChange, handleBlur, isSubmitting }) => (
            <Form className="space-y-6">
              <div className="relative">
                <label
                  htmlFor="oldPassword"
                  className="dark:text-white text-black text-sm font-medium mb-2 block"
                >
                  Enter old password
                </label>
                <Field
                  ...
                  className={`bg-neutral-100 dark:bg-neutral-800 border ${
                    touched.oldPassword && errors.oldPassword
                      ...
                      ? "border-green-500"
                      : "border-neutral-300 dark:border-neutral-800"
                  } text-black dark:text-white placeholder-dark dark:placeholder-gray-500 w-full rounded-lg px-4 py-2 focus:outline-none focus:ring-0 focus:ring-red-600`}
                  onChange={handleChange}
                  onBlur={handleBlur}
              ...
              <div className="flex justify-center">
                <Button
                  ...
                  className="px-8 py-3 bg-red-600 text-white font-bold rounded-md hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Change password
                </Button>
              </div>
