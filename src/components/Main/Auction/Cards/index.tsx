...
export const AuctionCards = ( { data } :AuctionCardsProps ) =>{
  ...
  return (
    <Links to={`/auction/${data.auctionId}`} className="w-[770px] rounded-xl border border-gray-400 bg-white dark:bg-[#2C2C2C] text-white flex overflow-hidden shadow-lg">
      {/* Ліва частина (фото) */}
      <div className="relative w-96 h-64">
    
        <img
          src={data?.mainImage || "https://placehold.co/680x350"}
...
<div className="absolute inset-0 bg-gradient-to-b from-black/60 to-black/10">
    ...
    <div className="absolute bottom-3 left-0 w-full px-3 flex justify-between">
        ...
        <div className="flex items-center gap-2 bg-neutral-800 border border-[#8ebf0b] px-3 py-1.5 rounded-lg text-xs font-medium font-synonym">
            ...
            {data.currentBid ?  (data?.currentBid.toLocaleString()) : "0"}
        </div>
    </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 flex flex-col justify-between">
        <div>
          <h3 className="text-black dark:text-white text-lg font-bold font-amulya">{data?.year} {data?.makeName} {data?.modelName}</h3>
          <p className="text-sm text-gray-500 dark:text-gray-300 mt-1 font-amulya">
            {`${data?.engine}, ${data?.transmission} `}
          </p>
        </div>
        ...
        {/* Локація */}
        <div className="text-sm text-gray-400 mt-2 font-amulya">{data?.location}</div>
